...
$imageDir = Join-Path $repoRoot 'assets\Images\program images\python images'
$output = Join-Path $repoRoot 'programs\python-gallery.json'
...
$items = Get-ChildItem -LiteralPath $imageDir -File |
    ...
    ForEach-Object {
        [pscustomobject]@{
            src = "../assets/Images/program images/python images/$($_.Name)"
            alt = Titleize $_.Name
        }
    }
